import argparse
from social_distance_estimation import calibration, getmap, pedestrian_detector, preprocess_frame, visualise_grid, visualise_main, calc_dist
import torch
...
def process(video, output_path, model_path):
    ...
    out = cv.VideoWriter(output_path, fourcc, 20.0,
    ...
    while True:
        ...
        if picId % 30 == 0:
            saveReportImages(image, picId, grid, warped)
        if picId == 300:
            break
        picId += 1
...
def saveReportImages(image, picId, grid, warped):
    ...
    # save figure
    plt.savefig(fname="./report/video/frames/" +
                "frame"+str(picId) + ".png", format="png")
    plt.clf()
...
if __name__ == '__main__':
    ...
    process(args.input, "./report/video/output.mp4", args.model)
