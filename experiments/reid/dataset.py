...
from path_cfg import MOTSYNTH_CSV, MOTSYNTH_IMG, MOT17_IMG, MOT17_CSV
...
def clean_rows(df, min_h, min_w, min_samples):
    ...
    clean_df['samples_per_id'] = clean_df.groupby('ped_id')['height'].transform('count').values
...
class MOTDataset(ImageDataset):
    ...
    def __init__(self, dataset_dir, img_dir, min_samples, min_h=50, min_w=25, **kwargs):
        ...
        to_tuple_list = lambda df: list(df[['image', 'ped_id', 'cam_id']].itertuples(index=False, name=None))
        df['cam_id'] = 0
...
def get_class(name, dataset_dir, img_dir):
    ...
    class MOT(MOTDataset):
        def __init__(self, **kwargs):
            super(MOT, self).__init__(dataset_dir=dataset_dir, img_dir=img_dir, min_samples=min_samples, **kwargs)
