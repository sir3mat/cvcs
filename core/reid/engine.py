...
from core.reid.deep_person_reid.torchreid.engine.image.softmax import ImageSoftmaxEngine
...
class ImageSoftmaxEngineSeveralSeq(ImageSoftmaxEngine):
    ...
    def test(
        ...
        if self.writer is not None:
            self.writer.add_scalar(f'Test/OVERALL/rank1',
                                   avg_rank1, self.epoch)
